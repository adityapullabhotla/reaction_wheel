...
# ==========================================
# OUTER LOOP: CAMERA VISION TRACKING (RECTANGLE)
# ==========================================
...
def find_target(frame):
    ...
    # We pass half the longest side so it doesn't crash the update_tracking_marker function
...
def track():
    ...
    print("\nCascaded Control Active (Green Rectangle Mode). Press Ctrl+C to stop and render graphs.")
    ...
    try:
        while system_running:
            ...
            if result is not None:
                ...
                target_x_history.append(target_cx)
                if len(target_x_history) > SMOOTH_FRAMES: target_x_history.pop(0)
                smoothed_cx = int(sum(target_x_history) / len(target_x_history))
                ...
                # Log Search Data (Use NaN for error so the graph line breaks nicely when target is lost)
                time_log.append(elapsed)
