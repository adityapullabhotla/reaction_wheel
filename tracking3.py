...
# ── PID ───────────────────────────────────────────────────────────────────────
Kp = 0.75   # proportional — main driver
Ki = 0.0   # integral — leave at 0 unless steady-state offset persists
Kd = 0.3   # derivative — damps oscillation; raise slowly if needed

# ── Motor output (%) ──────────────────────────────────────────────────────────
# Keep these LOW so the camera stays steady
PWM_DEAD   =  0.0   # below dead-zone: off
PWM_MIN    =  7.0   # minimum power that actually moves the platform
...
SEARCH_PWM = 3.5   # % — barely above stall
SEARCH_REVERSE_SEC = 1.8   # seconds before reversing sweep direction
